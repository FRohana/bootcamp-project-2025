...
export default function PortfolioCommentForm({
  projectName,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log("Submitting comment for project:", projectName);
  ...
  return (
    <form className="comment-form" onSubmit={handleSubmit}>
      ...
      <label htmlFor={`comment-user-${projectName}`}>Your name:</label>
      <input
        type="text"
        id={`comment-user-${projectName}`}
        name="user"
        value={user}
      ...
      <label htmlFor={`comment-text-${projectName}`}>Add your comment:</label>
      <textarea
        id={`comment-text-${projectName}`}
        name="comment"
        value={comment}
